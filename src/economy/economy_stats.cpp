...
float trade_supply(sys::state const& state,
	...
	auto merchants_supply = std::min(std::max(0.f, stockpiles) * stockpile_to_supply, state.world.market_get_aggregated_demand_history(m, c) * (0.85f + state.world.market_get_price(m, c) / state.world.commodity_get_median_price(c)) + 0.1f);
