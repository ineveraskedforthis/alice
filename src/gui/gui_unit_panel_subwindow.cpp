...
public:
	...
	void update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept override {
		...
		if(!base_pop) {
			...
			auto reinf = state.defines.pop_size_per_regiment * military::estimate_reinforcement<military::interval_estimation::monthly, military::supply_estimation::based_on_satisfaction, true>(state, reg_id);
			if(reinf >= 2.0f) {
